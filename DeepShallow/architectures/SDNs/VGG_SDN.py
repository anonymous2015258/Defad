...
class VGG_SDN(nn.Module):
    ...
    def forward(self, x):
        outputs = []
        interm = self.init_conv(x)
        #interm = fwd.detach().clone()
        i=0
        for layer in self.layers:
            if(i==0):
                #fwd
                fwd, is_output, output = layer(interm)
            else:
    ...
    def early_exit(self, x):
        ...
        for layer in self.layers:
            ...
            if is_output:
                ...
                if confidence >= self.confidence_threshold:
                    is_early = True
                    return output, output_id, is_early,interm
        ...
        is_early = False
        return outputs[max_confidence_output], max_confidence_output, is_early, interm