    ...
    def forward(self, x,bc):
        ...
        for ii in range(self.nScales):
            ...
            for i in range(self.nBlocks[ii+1]):
                ...
                if n_block_curr > self.nBlocks[ii]:
                    #print(self.m(self.classifier[classifier_idx](_x)))
                    temp=torch.max(self.m(self.classifier[classifier_idx](_x)), 1)

                    #print(temp[0].tolist())
                    #time.sleep(5)
                    for idx in range(bc):
                        #print(temp[0][idx].tolist())
                        #time.sleep(15)
                        if (temp[0][idx].tolist() > 0.9 and indexes[idx]==-1.0):

                            #print(classifier_idx)
                            #time.sleep(1)
                            indexes[idx] = torch.tensor(classifier_idx+1)
                            #res.append(temp)
                            #temp_bool = True
                            #break
                        #res.append(temp)
                        if(indexes[idx]==-1.0 and classifier_idx+1==8):
                            indexes[idx] = torch.tensor(8)
                    classifier_idx += 1
