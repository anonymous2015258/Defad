    ...
    def forward_single(self, x, policy):
        ...
        for segment, num_blocks in enumerate(self.layer_config):
           ...
           for b in range(num_blocks):
                ...
                t += 1
        '''p2.stop()
        eng, tm = p2.getTotalEnergy()
        energy.append(eng)
        with open('out3.csv', 'a') as fd:
            write_outfile = csv.writer(fd)
            write_outfile.writerow(energy)'''
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x
...
class Policy32(nn.Module):
    ...
    def forward(self, x):
        #print('in policy')
        x1 = self.features.forward_full(x)
        value = self.vnet(x1)
        probs = F.sigmoid(self.logit(x1))
        return probs, value,x1
    def get_grad(self, x,device):
        x1 = self.features.forward_full(x)
        value = self.vnet(x1)
        probs = F.sigmoid(self.logit(x1))
        target = torch.tensor([0.0] * 54).to(device)

        (target-probs).sum().backward()
        grads=self.vnet.weight.grad
        return probs, value,grads
